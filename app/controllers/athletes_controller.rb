...
class AthletesController < ApplicationController
  ...
  def set_result
    ...
    result_data = results.pluck(:official, :tournament_id, :established_date, :result, :wind) if %w[short field].include?(@type)
    ...
    gon.winds = %w[short field].include?(@type) ? result_data.map { |r| r[4].present? ? "(#{r[4] >= 0.0 ? '+' : ''}#{r[4]})" : '' } : Array.new(results.size, '')
  end
end
