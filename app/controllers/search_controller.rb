  ...
  def search_result(class_name)
    if class_name == 'RelayResult'
      if params[:athlete_id].present?
        result = RelayResult.where(first_athlete_id: params[:athlete_id]).or(
          RelayResult.where(second_athlete_id: params[:athlete_id])
        ).or(
          RelayResult.where(third_athlete_id: params[:athlete_id])
        ).or(
    ...
    if params[:established_from].present? || params[:established_to].present?
      ...
      result = if result.nil?
                 Object.const_get(class_name).where(established_date: from..to)
               else
                 result.where(established_date: from..to)
               end
    end
    ...
    if params[:competition].present?
      if result.nil?
        if class_name == 'DecathlonResult'
          ...
          result = Object.const_get(class_name).where(competition_id: params[:competition])
        end
      else
        result = result.where(competition_id: params[:competition]) unless class_name == 'DecathlonResult'
      end
    end
