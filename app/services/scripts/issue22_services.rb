...
module Scripts
  class Issue22Services
    ...
    def build_competition_id_column
      ...
      ShortResult.import(short_results, on_duplicate_key_update: { columns: [:competition_id], conflict_target: [:id] })
      LongResult.import(long_results, on_duplicate_key_update: { columns: [:competition_id], conflict_target: [:id] })
      FieldResult.import(field_results, on_duplicate_key_update: { columns: [:competition_id], conflict_target: [:id] })
      RelayResult.import(relay_results, on_duplicate_key_update: { columns: [:competition_id], conflict_target: [:id] })
    end
